...
def telemetry(sid, data):
    if data:
        ...
        image_height_orig =image_array.shape[0]
        # cropping out irrelevant part of the picture
        image2 = image_array[60:image_height_orig-30, :, :]
        # resize the image for the nvidia model
        image2 = cv2.resize(image2, (glob_image_width, glob_image_height), cv2.INTER_AREA)
        # convert to yuv space for nvidia model
        image2 = cv2.cvtColor(image2, cv2.COLOR_RGB2YUV)
            
        steering_angle = float(model.predict(image2[None, :, :, :], batch_size=1))
